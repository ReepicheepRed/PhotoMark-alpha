...
import com.sticker.app.camera.util.Point2D;
import com.sticker.app.camera.util.UIUtils;
...
public class MyHighlightView implements EditableDrawable.OnSizeChange {
    ...
    private int                   mAnchorEditHeight;
    private int                   mAnchorEditWidth;
    ...
    public MyHighlightView(ImageView context, int styleId, FeatherDrawable content) {
        ...
        mAnchorRotate = App.getApp().getResources().getDrawable(R.drawable.aviary_resize_knob);
        mAnchorDelete = App.getApp().getResources().getDrawable(R.drawable.aviary_delete_knob);
        mAnchorEdit = App.getApp().getResources().getDrawable(R.drawable.bianjikuang_btn_bianji);
        ...
        if (null != mAnchorEdit) {
            mAnchorEditWidth = mAnchorEdit.getIntrinsicWidth() / 2;
            mAnchorEditHeight = mAnchorEdit.getIntrinsicHeight() / 2;
        }
    ...
    public void draw(final Canvas canvas) {
        ...
        if (is_selected || is_focused) {
            ...
            if (mShowAnchors) {
                ...
                if (mAnchorEdit != null) {
                    mAnchorEdit.setBounds(right - mAnchorEditWidth, top - mAnchorEditHeight,
                            right + mAnchorEditWidth, top + mAnchorEditHeight);
                    mAnchorEdit.draw(canvas);
                }
    ...
    public void onSingleTapConfirmed(float x, float y) {
        ...
        if (mAnchorEdit != null) {
            if ((Math.abs(rect.right - x) < HIT_TOLERANCE)
                    && (Math.abs(rect.top - y) < HIT_TOLERANCE) && verticalCheck && horizCheck) {
    ...
    public void setOnEditClickListener(OnEditClickListener mEditClickListener) {
