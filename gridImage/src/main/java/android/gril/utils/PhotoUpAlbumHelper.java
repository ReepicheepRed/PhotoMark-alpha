	...
	void buildImagesBucketList() {
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				int end = cur.getString(photoPathIndex).contains(".") ?
						cur.getString(photoPathIndex).lastIndexOf(".") : cur.getString(photoPathIndex).length();
				if (end <= start || cur.getString(photoPathIndex).substring(start,end).replaceAll(" ", "").length()<=0)
				{
